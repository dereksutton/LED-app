    ...
    return (
        <section className="relative min-h-screen overflow-hidden">
            ...
            <div className="absolute inset-0 z-0">
                ...
                >
                    <source src={isMobile ? "/videos/led-hero-mobile.mp4" : "/videos/led-hero-optimized.mp4"} type="video/mp4" />
                </video>
                    ...
                    <header className="flex justify-between w-full px-4 lg:px-8">
                        ...
                        <nav className="hidden lg:flex flex-col items-end space-y-3 mt-4 pt-4 lg:pt-8">
                            ...
                            >
                                <Menu as="li" className="relative">
                                    {({ open }) => (
                                        <>
                                            ...
                                            >
                                                <Menu.Items static className="absolute -right-4 top-12 w-56 backdrop-blur-md bg-white/95 rounded-xl shadow-2xl py-2 focus:outline-none border border-slate-200 font-sans-luxury">
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury font-semibold no-underline`}>
                                                                All Services
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                                    <div className="mx-4 my-1 h-px bg-slate-200"></div>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/interior-painting/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury no-underline`}>
                                                                Interior Painting
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/exterior-painting/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury no-underline`}>
                                                                Exterior Painting
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/pressure-washing/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury no-underline`}>
                                                                Pressure Washing
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/cabinet-refinishing/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury no-underline`}>
                                                                Cabinet Refinishing
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/ceiling-services/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury no-underline`}>
                                                                Ceiling Services
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href="/services/commercial-painting/" className={`${active ? 'bg-slate-200 text-slate-800' : 'text-slate-800'} block px-5 py-3 transition-all duration-200 font-sans-luxury no-underline`}>
                                                                Commercial Painting
                                                            </Link>
                                                        )}
                                                    </Menu.Item>
                                ...
                                <li><Link href="/contact/" className="bg-slate-800 text-[--luxury-champagne] px-6 py-3 rounded-full font-sans-luxury font-semibold text-lg hover:bg-slate-700 hover:scale-105 transition-all duration-300 shadow-lg no-underline">Quote Request</Link></li>
                            </motion.ul>
                        </nav>
                            ...
                            <nav className="relative px-6 py-4">
                                <ul className="space-y-1">
                                    ...
                                    >
                                        <Link
                                            href="/services/"
                                            className="group relative block text-[#F5E6D3] text-xl font-serif-luxury py-2 hover:text-white transition-all duration-300 no-underline"
                                            onClick={() => setIsMenuOpen(false)}
                                        >
                                    ...
                                    >
                                        <Link
                                            href="/contact/"
                                            className="inline-block bg-gradient-to-r from-[#F5E6D3] via-[#E8B4A0] to-[#F5E6D3] text-[#2C3E50] px-6 py-3 rounded-full font-serif-luxury font-semibold text-lg hover:shadow-lg hover:shadow-[#F5E6D3]/20 transform hover:scale-105 transition-all duration-300 no-underline"
                                            onClick={() => setIsMenuOpen(false)}
                                        >
                                            Get Free Quote
                                        </Link>
